    ...
    <div className="mx-auto max-w-7xl px-4 py-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-serif text-3xl leading-none">Thailand 2026</h1>
...
function Title({title,subtitle}) {
  return <div><h2 className="font-serif text-4xl text-stone-950">{title}</h2><p className="mt-1 text-stone-600">{subtitle}</p></div>
}

function Stat({label,value}) {
  return <div className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-stone-200"><p className="text-xs font-black uppercase tracking-[0.2em] text-stone-500">{label}</p><p className="mt-2 text-3xl font-black text-emerald-950">{value}</p></div>
}
  ...
  return <div className="space-y-6">
    <section className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
      <div className="rounded-[2.25rem] bg-gradient-to-br from-emerald-950 via-emerald-800 to-lime-600 p-6 text-white shadow-xl">
        ...
        <h2 className="mt-3 font-serif text-5xl leading-tight">Save money everywhere. Spend only where the trip becomes real.</h2>
...
function RegionsTab() {
  ...
  {regions.map(r => <article key={r.name} className="grid overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-stone-200 lg:grid-cols-[360px_1fr]">
    ...
    <div className="p-6"><div className="flex flex-wrap gap-2"><Badge tone="blue">{r.dates}</Badge><Badge>{r.hotel}</Badge></div><h3 className="mt-4 font-serif text-3xl text-stone-950">{r.name}</h3><p className="mt-3 text-stone-700">{r.why}</p>
